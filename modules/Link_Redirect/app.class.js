	...
	*/
	...
	function triggers()
	{
		...
		$('body').on('click', ".psp-add-link-form input#psp-submit-to-builder", function(e){
			...
			if (url == url_redirect) {
				alert('URL & URL Redirect fields are identical!');
				return false;
			}
		...
		$('body').on('click', ".psp-do_item_delete", function(e){
			...
			var that = $(this),
				...
				id	= row.data('itemid'),
				url = row.find('td').eq(3).text(),//.find('input').val(),
				url_redirect = row.find('td').eq(4).text();//.find('input').val();

			//row.find('code').eq(0).text()
			if(confirm('Delete row with ID = ' + id + ' from redirect? This action can\'t be rollback!' )){
